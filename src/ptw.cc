...
void PTWclass::initialize() {
    ...
    for (uint32_t set = 0; set < unified_pwc.sets; set++) {
...
void PTWclass::update_lru(uint32_t set, uint32_t way) {
    ...
    for (uint32_t w = 0; w < unified_pwc.ways; w++) {
        if (w != way && unified_pwc.entries[set][w].valid) {
            unified_pwc.entries[set][w].lru++;
        }
    }
...
uint32_t PTWclass::find_victim(uint32_t set) {
    ...
    uint32_t max_lru = 0;
    uint32_t ways = unified_pwc.ways;
    
    // First, look for an invalid entry
    for (uint32_t way = 0; way < ways; way++) {
        if (!unified_pwc.entries[set][way].valid) {
            return way;
        }
    }
...
void PTWclass::operate() {
    // Queue any ready MSHR entries if space is available in outstanding walks
    for (int i = 0; i < n_mshr; i++) {
        PTW_MSHR_Entry &me = mshr[i];
        if (me.valid && me.ready) {
            if (is_walk_queue_available(me.vaddr)) {
                int next_level = me.current_level - 1;
                OutstandingWalk nw;
    ...
    for (size_t i = 0; i < outstanding_walks.size(); i++) {
        ...
        while (walk.current_level >= 0) {
            ...
            for (int mi = 0; mi < n_mshr; mi++) {
                if (mshr[mi].valid && !mshr[mi].ready
...
void PTWclass::handle_memory_response(PACKET *packet) {
    ...
    if (lvl >= 0) {
        ...
        for(int i = 0; i < n_mshr; i++) 
        {
            ...
            if(next_level >= 0) {
                me->ready = true;
                me->next_level_base = next_level_base;
            }
            else {
