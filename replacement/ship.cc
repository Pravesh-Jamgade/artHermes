...
void SHiPRepl::print_config()
{
    cout << "ship.SHIP_maxRRPV " << SHIP_maxRRPV << endl
        << "ship.SHIP_SHCT_SIZE " << SHIP_SHCT_SIZE << endl
        << "ship.SHCT_PRIME " << SHCT_PRIME << endl
        << "ship.SAMPLER_SET " << SAMPLER_SET << endl
        << "ship.SAMPLER_WAY " << SAMPLER_WAY << endl
        << "ship.SHCT_MAX " << SHCT_MAX << endl
        << endl;
}

// initialize replacement state
void SHiPRepl::initialize_replacement()
{
    // cout << "Initialize SHIP state" << endl;

    for (int i=0; i<LLC_SET; i++) {
        for (int j=0; j<LLC_WAY; j++) {
            rrpv[i][j] = SHIP_maxRRPV;
        }
    }
...
uint32_t SHiPRepl::is_it_sampled(uint32_t set)
{
    for (int i=0; i<SAMPLER_SET; i++)
        if (rand_sets[i] == set)
            return i;
...
// update sampler
void SHiPRepl::update_sampler(uint32_t cpu, uint32_t s_idx, uint64_t address, uint64_t ip, uint8_t type)
{
...
uint32_t SHiPRepl::find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
    // look for the SHIP_maxRRPV line
    while (1)
    {
        for (int i=0; i<LLC_WAY; i++)
            if (rrpv[set][i] == SHIP_maxRRPV)
                return i;
...
void SHiPRepl::update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
    string TYPE_NAME;
    if (type == LOAD)
        TYPE_NAME = "LOAD";
    ...
    if (hit)
        ...
        rrpv[set][way] = SHIP_maxRRPV-1;
        if (SHCT[cpu][SHCT_idx].counter == SHCT_MAX)
            rrpv[set][way] = SHIP_maxRRPV;
    }
}

// use this function to print out your own stats at the end of simulation
void SHiPRepl::dump_stats()
{
